...
class KageBunshinStateManager:
    ...
    def get_tools_for_llm(self):
        ...
        async def press_key(key: str) -> str:
            ...
            **Form Navigation:**
            ```
            press_key("Tab")        # Move to next form field
            press_key("Shift+Tab")  # Move to previous form field
            press_key("Enter")      # Submit form or activate button
            ```
            
            **Modal/Dialog Interaction:**
            ```
            press_key("Escape")     # Close modal or cancel dialog
            press_key("Enter")      # Confirm action in dialog
            ```
            
            **Page Navigation:**
            ```
            press_key("PageDown")   # Scroll down one page
            press_key("Home")       # Go to top of page
            press_key("End")        # Go to bottom of page
            ```
